...
def _resolve_type_ref(type_ref):
    """Resolve a type reference from a string or callable."""
...
class Argument(MountedType):
    ...
    Arguments will be parsed and provided to resolver methods as keyword arguments.
    """
    ...
    def __init__(
        ...
        if required:
            assert deprecation_reason is None, f"Argument {name} is required and cannot be deprecated."
            type_ = NonNull(type_)
    ...
    for default_name, arg in iter_arguments:
        ...
        if isinstance(arg, (InputField, Field)):
            raise ValueError(
                ...
                "Try using Argument({arg.type})."
            )
        if not isinstance(arg, Argument):
            raise ValueError(f'Unknown argument "{default_name}".')
        arg_name = default_name or arg.name
        assert arg_name not in arguments, f'More than one Argument have the name "{arg_name}".'
