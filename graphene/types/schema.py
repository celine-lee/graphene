...
def assert_valid_root_type(type_):
    ...
    assert is_graphene_objecttype or is_graphql_objecttype, f"Type {type_} is not a valid ObjectType."
...
class TypeMap(dict):
    ...
    def create_inputobjecttype(self, graphene_type):
        return GrapheneInputObjectType(
            ...
            fields=partial(self.create_fields_for_type, graphene_type, is_input_type=True),
        )
    ...
    def get_name(self, name):
        return to_camel_case(name) if self.auto_camelcase else name
    ...
    def create_fields_for_type(self, graphene_type, is_input_type=False):
        ...
        for name, field in graphene_type._meta.fields.items():
            if isinstance(field, Dynamic):
                dynamic_type = field.get_type(self)
                if isinstance(dynamic_type, MountedType):
                    field = dynamic_type
                elif isinstance(dynamic_type, UnmountedType):
                    field = Field.mounted(dynamic_type)
                else:
                    continue
                if not field:
                    continue
...
class Schema:
    ...
    A Graphene Schema can execute queries, mutations, or subscriptions against the defined types.
    """
