...
class TypeMap(dict):
    ...
    def _get_interfaces(self, graphene_type):
        interfaces = []
    ...
    def _get_resolve_type(self, graphene_type):
        if graphene_type.resolve_type:
            return partial(self.resolve_type, graphene_type.resolve_type, graphene_type._meta.name)
        return None

    def create_objecttype(self, graphene_type):
        def interfaces():
            return self._get_interfaces(graphene_type)
    ...
    def create_interface(self, graphene_type):
        resolve_type = self._get_resolve_type(graphene_type)
    ...
    def construct_union(self, graphene_type):
        def types():
            ...
            for graphene_objecttype in graphene_type._meta.types:
                object_type = self.add_type(graphene_objecttype)
    ...
    def create_fields_for_type(self, graphene_type, is_input_type=False):
        ...
        for name, field in graphene_type._meta.fields.items():
            if isinstance(field, Dynamic):
                field_val = field.get_type(self)
                if isinstance(field_val, MountedType):
                    field = field_val
                elif isinstance(field_val, UnmountedType):
                    field = Field.mounted(field_val)
                else:
                    continue
                if not field:
                    continue
    ...
    def resolve_type(self, resolve_type_func, type_name, root, info, _type):
        ...
        return default_type_resolver(root, info, self[type_name])
