    ...
    def create_fields_for_type(self, graphene_type, is_input_type=False):
        ...
        for name, field in graphene_type._meta.fields.items():
            if isinstance(field, Dynamic):
                inner_field = field.get_type(self)
                if isinstance(inner_field, MountedType):
                    field = inner_field
                elif isinstance(inner_field, UnmountedType):
                    field = Field.mounted(inner_field)
                else:
                    continue
                if not field:
                    continue
...
class Schema:
    """
    Schema Definition.
    A Graphene Schema can execute queries, mutations, and subscriptions against the defined types.
    """
